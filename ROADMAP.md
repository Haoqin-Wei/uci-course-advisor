...
- 一条命令可以执行全部离线测试。
- 测试不需要 API key、不调用 Anteater、不调用真实 LLM。
- 核心测试失败时返回非零退出状态。
- 后续 M2–M6 的每个 bug 修复先增加失败测试，再修改实现。
...
- [ ] 修复 `load_student_into_session()` 对 `get_student_profile()` envelope 的错误读取。
- [ ] 新会话创建 `sess_xxx` 后，当前请求立即切换到该 ID；禁止使用空字符串保存第一轮状态。
